...
if show_quiz:
    ...
    # 解答選択肢をトグル風に表示import streamlit as st
import pandas as pd
import numpy as np
import time
...
if tab_selection == "第一章、第二章":
    ...
    if not st.session_state.answer_button_disabled:
        if st.button('解答する'):
            ...
            # 正誤判定とフィードバックの表示
            selected_choices = [choice for choice in st.session_state.choices if st.checkbox(choice)]
            if len(selected_choices) == 1 and selected_choices[0] == st.session_state.correct_answer:
                st.session_state.score += 10
                st.success("正解です！")
            else:
                st.session_state.score = max(st.session_state.score - 10, 0)
